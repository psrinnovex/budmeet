...
export default function Features() {
  return (
    <section id="features" className="relative isolate overflow-hidden" style={{ backgroundColor: BRAND.bg }}>
      ...
      <div className="relative mx-auto max-w-6xl px-4 pb-10 pt-16 sm:pt-20 md:pt-24">
        <div className="text-center">
          ...
          <p className="mx-auto mt-2 max-w-xl text-sm text-white/70">
            Built for real life. Tap a vibe, meet today.
          </p>
        </div>
      ...
      <style jsx global>{`
        @media (prefers-reduced-motion: reduce) {
          canvas, [style*="animation"], [class*="animate"], [class*="transition"] {
...
function ImageFeature({ src, title, caption }: { src: string; title: string; caption: string }) {
  return (
...
function SceneAurora() {
  ...
  return (
    <>
      ...
      <AuroraRibbon position={[0, 1.4, -2]} rotation={[0.1, 0.2, 0]} size={[12, 3]} colorA={BRAND.green} colorB={BRAND.blue} distort={0.45} speed={0.9 * energy} opacity={0.6} />
      <AuroraRibbon position={[-1, -1.2, -1.5]} rotation={[0.05, -0.3, 0.1]} size={[11, 2.6]} colorA={BRAND.blue} colorB={BRAND.green} distort={0.38} speed={0.7 * energy} opacity={0.5} />
      <AuroraRibbon position={[1.2, 0.3, -3]} rotation={[0.12, 0.28, -0.06]} size={[13, 2.2]} colorA={BRAND.green} colorB={BRAND.blue} distort={0.32} speed={0.6 * energy} opacity={0.45} />

      <OrbitNode color={BRAND.green} radius={4.8} speed={0.8 * energy} phase={0} />
...
function OrbitNode({ color, radius, speed, phase = 0 }: { color: string; radius: number; speed: number; phase?: number }) {
  const ref = React.useRef<THREE.Mesh>(null!);
  useFrame((state) => {
    const t = state.clock.getElapsedTime() * speed + phase;
    ref.current.position.set(Math.cos(t) * radius, Math.sin(t * 1.25) * (radius * 0.16) + 0.3, Math.sin(t) * radius);
  });
  ...
  return (
    <Trail width={0.14} color={new THREE.Color(color)} length={26} decay={0.8} attenuation={(w) => w}>
      <mesh ref={ref}>
        ...
        <meshStandardMaterial color={color} emissive={new THREE.Color(color)} emissiveIntensity={1.2} metalness={0.5} roughness={0.25} />
      </mesh>
    </Trail>
  );
}
